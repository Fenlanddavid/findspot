...
| `services-eu1.arcgis.com` | Historic England AIM project-coverage query; AIM/NHLE fallback queries | GET FeatureServer | No | Yes | No | No | The AIM project-area layer receives the current scan bounding box so FindSpot can distinguish mapped, partly mapped and unmapped ground. Monument features normally use the static R2 path; direct feature queries are fallbacks |
