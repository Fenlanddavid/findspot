...
export function createFieldGuideMapStyle(isSatellite = false): maplibregl.StyleSpecification {
    return {
        ...
        sources: {
            ...
            'overlay-relief': { type: 'raster', tiles: ['https://services.arcgisonline.com/ArcGIS/rest/services/Elevation/World_Hillshade/MapServer/tile/{z}/{y}/{x}'], tileSize: 256, minzoom: 0, maxzoom: 16, attribution: 'Esri World Hillshade · multi-directional relief' },
        ...
        layers: [
            ...
            { id: 'overlay-relief', type: 'raster', source: 'overlay-relief', layout: { visibility: 'none' }, paint: { 'raster-opacity': 0.85, 'raster-contrast': 0.55, 'raster-saturation': -1, 'raster-brightness-max': 0.7, 'raster-fade-duration': 0 } },
...
export function registerRomanStandaloneLayers(map: maplibregl.Map): boolean {
    if (map.getSource('roman-roads-standalone')) return false;
    ...
    map.addLayer({
        ...
        paint: { 'line-color': '#ffffff', 'line-width': 5.5, 'line-opacity': 0.35, 'line-dasharray': [5, 3] },
    });
    map.addLayer({
        id: 'roman-standalone',
        type: 'line',
        ...
        minzoom: ROMAN_STANDALONE_MIN_ZOOM,
        layout: { visibility: 'none', 'line-join': 'round', 'line-cap': 'round' },
        paint: { 'line-color': '#3b82f6', 'line-width': 2.5, 'line-opacity': 0.97, 'line-dasharray': [5, 3] },
    });
    return true;
}
