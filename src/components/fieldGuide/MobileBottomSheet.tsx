...
function LandscapeInterpretationPanel({
    ...
    return (
        <div className="rounded-xl border border-sky-400/20 bg-sky-500/[0.06] overflow-hidden">
            ...
            >
                <span className="text-[0.5625rem] font-black text-sky-400/75 uppercase tracking-[0.16em]">Terrain Reading</span>
                <svg
                    width="10"
                    height="10"
                    viewBox="0 0 24 24"
                ...
                <div className={`px-3 pb-3 animate-in fade-in duration-150 ${chips.length ? 'space-y-2' : ''}`}>
                    {chips.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                            {chips.map(chip => (
                                <span key={chip} className="text-[0.5625rem] font-bold text-sky-200/75 bg-sky-400/10 border border-sky-400/20 px-1.5 py-0.5 rounded">
                                    {chip}
                                </span>
                            ))}
                        </div>
                    )}
                    <p className="text-[0.6875rem] font-bold text-sky-100/82 leading-relaxed">{narrative}</p>
                </div>
            )}
        </div>
    );
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {historicMode ? (
                                    <button
                                        ...
                                        className={`rounded-xl border px-3 py-2 text-[0.5625rem] font-black uppercase tracking-widest transition-colors ${expandedGeologyId === 'historic' ? 'bg-emerald-500/20 border-emerald-400/40 text-emerald-200' : 'bg-white/[0.04] border-sky-400/20 text-sky-300'}`}
                                    >
                                        Geology
                                    </button>
                                ) : (
                                ...
                                <div className="rounded-xl border border-white/10 bg-slate-950/85 p-1.5 shadow-[0_10px_28px_rgba(0,0,0,0.24)] animate-in fade-in slide-in-from-top-1 duration-150">
                                    {HISTORIC_LAYER_OPTIONS.map(({ key, label }) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span className="text-[0.625rem] font-black uppercase tracking-widest leading-tight">{label}</span>
                    ...
                    <>
                        {scheduledMonumentCheckFailed && hasScanned && (
                            <div className="rounded-xl border border-amber-400/25 bg-amber-500/[0.08] px-3 py-2">
                                ...
                                <p className="text-[0.625rem] font-bold text-amber-100/75 leading-snug">Protected monument data could not be checked for this scan. Treat the result as incomplete and verify official records before fieldwork.</p>
                            </div>
                        )}
                        <HotspotTray />
                    </>
                    ...
                    return (
                        <div className="space-y-3">
                            <div className="flex items-start gap-3">
                                <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border border-white/10">
                                    {selectedUserFindMedia
                                        ...
                                        : <div className="w-full h-full border border-dashed border-white/15 rounded-xl grid place-items-center text-[0.5625rem] font-black text-white/20 uppercase tracking-wider">No Photo</div>
                                    }
                                </div>
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                        <span className={`px-2 py-0.5 rounded-full text-[0.5625rem] font-black uppercase tracking-widest ${chipClass}`}>{selectedUserFind.period}</span>
                                        {selectedUserFind.material && <span className="text-[0.625rem] text-white/40">{selectedUserFind.material}</span>}
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {selectedUserFind.weightG != null && <span className="text-[0.625rem] text-white/40">{selectedUserFind.weightG} g</span>}
                            </div>
                            {(() => {
                                ...
                                return (
                                    <div className="flex items-start gap-2 rounded-xl bg-teal-500/6 border border-teal-500/15 px-2.5 py-2 mt-1">
                                        <span className="text-teal-400 text-[0.5625rem] shrink-0 mt-0.5">◆</span>
                                        <div className="min-w-0">
                                            <p className="text-[0.5rem] font-black text-teal-300/80 uppercase tracking-widest mb-0.5">
                                                FieldGuide{annotation.status === 'within' ? ' · Inside zone' : ' · Nearby'}
                                            </p>
                                            <p className="text-[0.5625rem] font-bold text-white/65 leading-snug">{annotation.note}</p>
                                        </div>
                                    </div>
                                );
                            })()}
                            {selectedUserFind.notes?.trim() && (
                                <p className="text-[0.6875rem] text-white/40 italic leading-snug line-clamp-3">{selectedUserFind.notes.trim()}</p>
                            )}
                            <div className="border-t border-white/8 pt-2">
                                <span className="text-[0.625rem] text-white/25 font-mono">{selectedUserFind.findCode}</span>
                            </div>
                        </div>
                    );
                })()}
                ...
                {selectedPASFind && !selectedUserFind && (
                    <div className="space-y-3">
                        <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                                <p className="text-[0.5rem] font-black text-emerald-400 uppercase tracking-[0.2em] mb-1">Heritage Feature</p>
                                <h3 className="text-sm font-black text-white tracking-tight leading-tight">{selectedPASFind.objectType}</h3>
                                <p className="text-[0.6875rem] font-black text-emerald-400 mt-0.5">{selectedPASFind.broadperiod}</p>
                            </div>
                            ...
                            <p className="text-[0.6875rem] font-bold text-white/70 leading-snug">Standing heritage feature recorded in the OpenStreetMap community dataset.</p>
                            <a
                                href={`https://www.openstreetmap.org/${selectedPASFind.osmType || 'node'}/${selectedPASFind.internalId}`}
                                target="_blank" rel="noreferrer"
                            className="flex items-center justify-center gap-2 w-full py-3 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 rounded-2xl text-[0.625rem] font-black uppercase tracking-widest transition-all active:scale-[0.98]"
                        >
                ...
                {selectedId && !selectedHotspotId && !selectedUserFind && !selectedPASFind && displayTargets.filter(f => f.id === selectedId).map(f => {
                    ...
                    return (
                        <div key={f.id} className="space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    ...
                                    {isPrimaryTarget && (
                                        <button
                                            ...
                                            className="mb-1 inline-flex items-center gap-1.5 rounded-full border border-emerald-200/45 bg-[linear-gradient(135deg,rgba(6,78,59,0.92),rgba(15,118,110,0.72)_55%,rgba(245,158,11,0.34))] px-2.5 py-1 text-[0.4375rem] font-black uppercase tracking-widest text-emerald-50 shadow-[0_0_0_1px_rgba(16,185,129,0.15),0_8px_22px_rgba(0,0,0,0.26),inset_0_1px_0_rgba(255,255,255,0.22)] active:scale-[0.98]"
                                        >
                                            <span className="h-1.5 w-1.5 rounded-full bg-amber-300 shadow-[0_0_10px_rgba(251,191,36,0.9)]" />
                                            Start Here
                                        </button>
                                    )}
                                    <p className="inline-flex rounded-full border border-amber-300/25 bg-amber-300/8 px-2 py-0.5 text-[0.5rem] font-black text-amber-100/85 uppercase tracking-[0.16em] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">Target {f.number.toString().padStart(2, '0')}</p>
                            ...
                            <p className="text-[0.6875rem] font-bold text-white/50 leading-snug">{tInterp.hook}</p>

                                    {(() => {
                                        const ctx = targetFindContext.get(f.id);
                                    if (!ctx) return null;
                                        return ctx.status === 'within'
                                            ? <p className="text-[0.5625rem] font-black text-emerald-400/80 uppercase tracking-widest">{ctx.count} find{ctx.count !== 1 ? 's' : ''} recorded here — signal supported</p>
                                            : <p className="text-[0.5625rem] font-black text-emerald-400/80 uppercase tracking-widest">{ctx.count} find{ctx.count !== 1 ? 's' : ''} recorded nearby</p>;
                                    })()}
                                {f.isHighConfidenceCrossing && (
                                    <div className="bg-blue-600/30 p-2 rounded-xl border border-blue-400/70 animate-pulse">
                                        <p className="text-[0.625rem] font-black uppercase text-white text-center tracking-[0.18em]">Likely historic crossing point</p>
                                </div>
                            )}
                            {f.explanationLines && f.explanationLines.length > 0 && (
                                <div className="border-t border-white/8 pt-3">
                                    <p className="text-[0.5rem] font-black text-white/40 uppercase tracking-widest mb-2">Why this matters</p>
                            ...
                            <div className="border-t border-emerald-500/15 pt-2">
                                <p className="text-[0.5rem] font-black text-emerald-500/70 uppercase tracking-[0.12em] mb-1">Target focus</p>
                                ...
                                <div className="border-t border-white/8 pt-2">
                                    ...
                                    {expandedTargetId === f.id && (
                                        <div id={`mobile-target-details-${f.id}`} className="mt-3 space-y-3 animate-in fade-in duration-200">
                                            <div>
                                            <p className="text-[0.5rem] font-black text-white/55 uppercase tracking-[0.15em] mb-1">Summary</p>
                                            <p className="text-[0.6875rem] text-white/85 leading-relaxed">{tInterp.summary}</p>
                                        </div>
                                        <div>
                                            <p className="text-[0.5rem] font-black text-white/55 uppercase tracking-[0.15em] mb-1">Why it stands out</p>
                                            <p className="text-[0.6875rem] text-white/85 leading-relaxed">{tInterp.whyItStandsOut}</p>
                                        </div>
                                        <div>
                                            <p className="text-[0.5rem] font-black text-white/55 uppercase tracking-[0.15em] mb-1">How to approach it</p>
                    ...
                    {neutralMobileGeologyAvailable && (
                        <button
                            ...
                            className={`w-full rounded-xl border px-3 py-2.5 text-[0.5625rem] font-black uppercase tracking-widest transition-colors ${expandedGeologyId === 'scan' ? 'bg-emerald-500/15 border-emerald-400/30 text-emerald-300' : 'bg-white/[0.04] border-sky-400/20 text-sky-300'}`}
                        >
                            Geology
                        </button>
                    )}
                ...
                {selectedMonument !== undefined && !selectedUserFind && !selectedPASFind && !selectedId && (
                    <div className="space-y-3">
                        ...
                        <div className="rounded-xl bg-stone-900/40 border border-stone-700/40 p-3 space-y-2">
                            ...
                            <p className="text-[0.6875rem] font-bold text-stone-300/60 leading-snug">Metal detecting, excavation, or intrusive activity may require legal consent. Avoid disturbing the site boundary and check current protections before any fieldwork.</p>
                        </div>
                    </div>
                )}
                ...
                {selectedMonument === undefined && selectedHotspotId && (() => {
                    ...
                    return (
                        <div className="space-y-3">
                            <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                    <p className="text-[0.4375rem] font-black text-white/30 uppercase tracking-widest mb-0.5">{HOTSPOT_TITLES[h.classification]} · Hotspot {h.number}</p>
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <p className={`text-sm font-black leading-tight ${hStrengthColour}`}>{hierarchy.signalStrength}</p>
                                        {isPrimaryHotspot && <span className="bg-emerald-500/15 border border-emerald-400/30 text-emerald-200 px-1.5 py-0.5 rounded-full text-[0.4375rem] font-black uppercase tracking-widest">Priority</span>}
                                    </div>
                                </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button
                            ...
                            <div className="space-y-2">
                                <div>
                                    <p className="text-[0.4375rem] font-black text-white/30 uppercase tracking-[0.18em] mb-0.5">Why it matters</p>
                                    <p className="text-xs font-bold text-white/85 leading-snug">{hierarchy.whyItMatters}</p>
                                </div>
                                <div>
                                    <p className="text-[0.4375rem] font-black text-emerald-400/60 uppercase tracking-[0.18em] mb-0.5">Interpretive cue</p>
                                    <p className="text-[0.6875rem] font-bold text-emerald-300 leading-snug">{hierarchy.nextAction}</p>
                                </div>
                            </div>
                            ...
                            {(() => {
                                ...
                                return (
                                    ...
                                    {h.isHighConfidenceCrossing && <div className="bg-blue-600/30 p-2 rounded-xl border border-blue-400/70 animate-pulse"><p className="text-[0.625rem] font-black uppercase text-white text-center tracking-[0.18em]">Likely historic crossing point</p></div>}
                            {h.disturbanceRisk === 'High' && <div className="bg-red-500/15 p-2 rounded-xl border border-red-400/30"><p className="text-[0.5625rem] font-black uppercase text-red-300 tracking-widest">Disturbed ground — interpret with caution</p></div>}
                            <div className="border-t border-white/8 pt-3">
                                <p className="text-[0.5rem] font-black text-white/40 uppercase tracking-widest mb-2">Evidence</p>
                                <div className="space-y-1.5">
                                ...
                                return (
                                    <div className={`mt-2 rounded-xl border px-3 py-2.5 space-y-1 ${isValidates ? 'bg-emerald-500/8 border-emerald-500/20' : 'bg-sky-500/6 border-sky-500/15'}`}>
                                        <div className="flex items-center gap-2">
                                            <span className={`text-[0.4375rem] font-black uppercase tracking-widest shrink-0 ${isValidates ? 'text-emerald-400' : 'text-sky-400'}`}>
                                                Your finds
                                            </span>
                                            <span className={`text-[0.5625rem] font-black ${isValidates ? 'text-emerald-200' : 'text-sky-200'}`}>
                                                {feedback.label}
                                            </span>
                                        </div>
                                        <p className="text-[0.5625rem] font-bold text-white/65 leading-snug">{feedback.note}</p>
                                    </div>
                                );
                            })()}
                            {(() => {
                                ...
                                return (
                                    <div className="mt-2 rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-2.5 space-y-1">
                                        ...
                                        <p className="text-[0.5625rem] font-bold text-white/65 leading-snug">{persisted.note}</p>
                                    </div>
                                );
                            })()}
                            {h.suggestedFocus && (
                                <div className="pt-2 border-t border-emerald-500/15">
                                    <p className="text-[0.5rem] font-black text-emerald-500/70 uppercase tracking-[0.12em] mb-1">Field focus</p>
                                    ...
                                    const breakdown = [{ label: 'Anomaly', val: h.metrics.anomaly, cap: 30 }, { label: 'Context', val: h.metrics.context, cap: 25 }, { label: 'Convergence', val: h.metrics.convergence, cap: 20 }, { label: 'Behaviour', val: h.metrics.behaviour, cap: 15 }];
                                        return (
                                            <div id={`mobile-hotspot-details-${h.id}`} className="mt-3 space-y-3 animate-in fade-in duration-200">
                                                <p className="text-[0.5rem] font-black text-white/25 uppercase tracking-[0.2em]">{getInterpretationLabel(h.confidence)}</p>
                                            ...
                                            <div className="space-y-1.5 pt-2 border-t border-white/10">
                                                {breakdown.map(({ label, val, cap }) => (
                                                    <div key={label} className="flex items-center gap-2">
                                                        <span className="text-[0.4375rem] text-white/45 w-16 shrink-0">{label}</span>
                                                        ...
                                                        <span className="text-[0.4375rem] text-white/40 w-14 text-right shrink-0">{getSignalBand(val, cap)}</span>
                                                    </div>
                                                ))}
                                                {h.metrics.penalty !== 0 && <p className="text-[0.4375rem] text-white/35 mt-1">Modern disturbance or noise was discounted before interpretation.</p>}
                                            </div>
                                        </div>
                                    );
                                    })()}
                                </div>
                                    <p className="text-center text-[0.4375rem] text-white/40 italic">Highlights historic activity — not guaranteed finds.</p>
                        </div>
                    );
                })()}
