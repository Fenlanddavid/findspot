...
function LandscapeInterpretationPanel({
    ...
    return (
        <div className="rounded-xl border border-sky-400/20 bg-sky-500/[0.06] overflow-hidden">
            ...
            >
                <span className="text-sm font-bold text-sky-400/75 uppercase tracking-[0.16em]">Terrain Reading</span>
                <svg
                    width="10"
                    height="10"
                    viewBox="0 0 24 24"
                ...
                <div className={`px-3 pb-3 animate-in fade-in duration-150 ${chips.length ? 'space-y-2' : ''}`}>
                    {chips.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                            {chips.map(chip => (
                                <span key={chip} className="text-sm font-bold text-sky-200/75 bg-sky-400/10 border border-sky-400/20 px-1.5 py-0.5 rounded">
                                    {chip}
                                </span>
                            ))}
                        </div>
...
export function MobileBottomSheet() {
    ...
    return (
        ...
        <div
            ...
            className={`absolute bottom-3 left-3 right-3 z-[91] flex flex-col bg-black/95 border border-white/12 rounded-2xl shadow-2xl backdrop-blur-xl overflow-hidden overscroll-contain transition-[max-height] duration-300 ease-out ${sheetExpanded ? 'max-h-[65dvh]' : 'max-h-[min(19rem,60dvh)]'} ${helperActive && helperTipIndex === 1 ? 'ring-2 ring-blue-300/45' : ''}`}
        >
            {/* Handle + Status + Actions — always visible */}
            <div
                data-testid="fieldguide-mobile-sheet-handle"
                className={`shrink-0 px-4 pt-2 pb-3 border-b border-white/5 cursor-pointer select-none touch-none flex flex-col gap-2.5 transition-[height] duration-300 h-auto`}
            ...
            >
                ...
                <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                        ...
                        <p className={`text-sm font-medium leading-snug break-words ${subIsHighlighted ? 'text-amber-400' : 'text-white/75'}`}>
                            {headerSub}
                        </p>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0 pt-0.5">
                        <button type="button" aria-label={sheetExpanded ? 'Collapse landscape panel' : 'Expand landscape panel'} aria-expanded={sheetExpanded} onClick={event => { event.stopPropagation(); persistSheetExpanded(!sheetExpanded); }} className="min-w-11 min-h-11 rounded-lg border border-white/10 bg-white/[0.04] grid place-items-center">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" className={`text-white/75 transition-transform duration-300 ${sheetExpanded ? 'rotate-180' : ''}`}><polyline points="6 9 12 15 18 9"/></svg>
                        </button>
                    </div>
                </div>
                <ScanControlPanel actions="primary" showViews={false} />
            </div>
            ...
            <div
                ...
                className={sheetExpanded ? 'flex-1 overflow-y-auto overscroll-contain touch-pan-y scrollbar-hide px-3 py-3 space-y-4' : 'hidden'}
            >

                {sheetExpanded && <ScanControlPanel actions="none" />}
                {sheetExpanded && <details className="rounded-xl border border-white/15 p-2"><summary className="min-h-11 cursor-pointer text-sm font-medium text-white/80">Scan controls</summary><ScanControlPanel actions="secondary" showViews={false} />
                        <button
                            onClick={e => { e.stopPropagation(); setShowFieldsPicker(v => !v); setShowLayerPicker(false); }}
                            className={`min-h-11 px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${showFields !== false ? 'bg-emerald-500/15 border-emerald-400/30 text-emerald-300' : 'bg-white/[0.04] border-white/10 text-emerald-400'}`}
                        >
                            {myFieldsLabel}
                        </button>
                </details>}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {historicMode ? (
                                    <button
                                        ...
                                        className={`rounded-xl border px-3 py-2 text-sm font-bold uppercase tracking-wide transition-colors ${expandedGeologyId === 'historic' ? 'bg-emerald-500/20 border-emerald-400/40 text-emerald-200' : 'bg-white/[0.04] border-sky-400/20 text-sky-300'}`}
                                    >
                                        Geology
                                    </button>
                                ) : (
                                ...
                                <button
                                    ...
                                    className={`rounded-xl border px-3 py-2 text-sm font-bold uppercase tracking-wide transition-colors ${intelLayersOpen ? 'bg-amber-500/20 border-amber-400/40 text-amber-200' : 'bg-white/[0.04] border-white/10 text-amber-400'}`}
                                >
                                    Layers
                                </button>
                            </div>
                                ...
                                <div className="rounded-xl border border-white/10 bg-slate-950/85 p-1.5 shadow-[0_10px_28px_rgba(0,0,0,0.24)] animate-in fade-in slide-in-from-top-1 duration-150">
                                    {HISTORIC_LAYER_GROUPS.map(group => (
                                        <div key={group.label} className="py-1">
                                            <p className="px-3 pb-1 pt-1 text-sm font-bold uppercase tracking-wide text-slate-600">{group.label}</p>
                ...
                {selectedUserFind && (() => {
                    const PERIOD_CHIP: Record<string, string> = {
                        ...
                        'Unknown': 'bg-white/5 text-white/75',
                    };
                    ...
                    return (
                        <div className="space-y-3">
                            <div className="flex items-start gap-3">
                                <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border border-white/10">
                                    {selectedUserFindMedia
                                        ...
                                        : <div className="w-full h-full border border-dashed border-white/15 rounded-xl grid place-items-center text-sm font-bold text-white/20 uppercase tracking-wider">No Photo</div>
                                    }
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between">
                                        <h3 className="text-lg font-bold text-white tracking-tight leading-tight mb-1 pr-2">
                                            {selectedUserFind.objectType || 'Unknown Object'}
                                        </h3>
                                        <button aria-label="Close selection" onClick={() => setSelectedUserFind(null)} className="min-h-11 min-w-11 bg-white/[0.04] hover:bg-white/[0.08] text-white/75 hover:text-white rounded-full p-1.5 transition-colors border border-white/10 flex-shrink-0 -mt-0.5">
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                        </button>
                                    </div>
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                        <span className={`px-2 py-0.5 rounded-full text-sm font-bold uppercase tracking-wide ${chipClass}`}>{selectedUserFind.period}</span>
                                        {selectedUserFind.material && <span className="text-sm text-white/75">{selectedUserFind.material}</span>}
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {selectedUserFind.weightG != null && <span className="text-sm text-white/75">{selectedUserFind.weightG} g</span>}
                            </div>
                            {(() => {
                                ...
                                return (
                                    <div className="flex items-start gap-2 rounded-xl bg-teal-500/6 border border-teal-500/15 px-2.5 py-2 mt-1">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-sm font-bold text-teal-300/80 uppercase tracking-wide mb-0.5">
                                                FieldGuide{annotation.status === 'within' ? ' · Inside zone' : ' · Nearby'}
                                            </p>
                                            <p className="text-sm font-bold text-white/75 leading-snug">{annotation.note}</p>
                                        </div>
                                    </div>
                                );
                            })()}
                            {selectedUserFind.notes?.trim() && (
                                <p className="text-xs text-white/75 italic leading-snug line-clamp-3">{selectedUserFind.notes.trim()}</p>
                            )}
                            <div className="border-t border-white/8 pt-2">
                                <span className="text-sm text-white/75 font-mono">{selectedUserFind.findCode}</span>
                            </div>
                        </div>
                    );
                })()}
                ...
                {selectedPASFind && !selectedUserFind && (
                    <div className="space-y-3">
                        <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                                <p className="text-sm font-bold text-emerald-400 uppercase tracking-[0.2em] mb-1">Heritage Feature</p>
                                <h3 className="text-base font-bold text-white tracking-tight leading-tight">{selectedPASFind.objectType}</h3>
                                <p className="text-xs font-bold text-emerald-400 mt-0.5">{selectedPASFind.broadperiod}</p>
                            </div>
                            <button aria-label="Close selection" onClick={() => setSelectedPASFind(null)} className="min-h-11 min-w-11 bg-white/[0.04] hover:bg-white/[0.08] text-white/75 hover:text-white rounded-full p-1.5 transition-colors border border-white/10 shrink-0">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                            </button>
                            </div>
                            ...
                            className="flex items-center justify-center gap-2 w-full py-3 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 rounded-2xl text-sm font-bold uppercase tracking-wide transition-all active:scale-[0.98]"
                        >
                ...
                {selectedId && !selectedHotspotId && !selectedUserFind && !selectedPASFind && displayTargets.filter(f => f.id === selectedId).map(f => {
                    ...
                    const strengthColour: Record<TargetSignalStrength, string> = {
                        'Strong Signal': 'text-amber-400', 'Moderate Signal': 'text-emerald-400', 'Supporting Signal': 'text-white/75',
                    };
                    if (f.isProtected) {
                        ...
                        <div key={f.id} className="space-y-3">
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <p className="text-sm font-bold text-stone-400/70 uppercase tracking-[0.2em] mb-1">{protectedCopy.label}</p>
                                    {f.aimInfo && <h3 className="text-base font-bold text-white/90 tracking-tight leading-tight">{f.aimInfo.type}</h3>}
                                </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button
                                            type="button"
                                            onClick={(e) => { e.stopPropagation(); toggleGeologyDetails(`target:${f.id}`); }}
                                            className={`text-sm font-bold uppercase tracking-wide transition-colors ${expandedGeologyId === `target:${f.id}` ? 'text-emerald-300' : 'text-sky-300 hover:text-sky-200'}`}
                                        >
                                            Geology
                                        </button>
                                        <button aria-label="Close selection" onClick={() => setSelectedId(null)} className="min-h-11 min-w-11 bg-white/[0.04] hover:bg-white/[0.08] text-white/75 hover:text-white rounded-full p-1.5 transition-colors border border-white/10">
                            ...
                            <div className="rounded-xl bg-stone-900/40 border border-stone-700/40 p-3 space-y-2">
                                ...
                                {f.aimInfo && (
                                    <div className="p-2 rounded-xl border bg-stone-900/30 border-stone-700/30">
                                        <p className="text-sm font-bold uppercase text-stone-400/60 leading-tight mb-0.5">Recorded designation</p>
                                        <p className="text-sm font-bold text-stone-200/70 leading-tight">{f.aimInfo.type} · {f.aimInfo.period}</p>
                                    </div>
                                )}
                            </div>
                            );
                    ...
                    return (
                        <div key={f.id} className="space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    ...
                                    {isPrimaryTarget && (
                                        <button
                                            ...
                                            className="mb-1 inline-flex items-center gap-1.5 rounded-full border border-emerald-200/45 bg-[linear-gradient(135deg,rgba(6,78,59,0.92),rgba(15,118,110,0.72)_55%,rgba(245,158,11,0.34))] px-2.5 py-1 text-sm font-bold uppercase tracking-wide text-emerald-50 shadow-[0_0_0_1px_rgba(16,185,129,0.15),0_8px_22px_rgba(0,0,0,0.26),inset_0_1px_0_rgba(255,255,255,0.22)] active:scale-[0.98]"
                                        >
                                            <span className="h-1.5 w-1.5 rounded-full bg-amber-300 shadow-[0_0_10px_rgba(251,191,36,0.9)]" />
                                            Start Here
                                        </button>
                                    )}
                                    <p className="inline-flex rounded-full border border-slate-200/25 bg-slate-900/45 px-2 py-0.5 text-sm font-bold text-slate-100 uppercase tracking-[0.14em] shadow-[inset_0_1px_0_rgba(255,255,255,0.10)]">T{f.number.toString().padStart(2, '0')}</p>
                                    <h3 className="text-base font-bold text-white tracking-tight leading-tight mt-0.5">{f.type}</h3>
                                    <p className={`text-sm font-bold mt-0.5 ${strengthColour[tInterp.signalStrength]}`}>{tInterp.signalStrength}</p>
                                </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button
                                            type="button"
                            ...
                            <p className="text-sm font-bold text-white/85 leading-snug">{getTargetVerdict(tInterp.signalStrength, isPrimaryTarget)}</p>
                            <p className="text-xs font-bold text-white/75 leading-snug">{tInterp.hook}</p>

                                    {(() => {
                                        const ctx = targetFindContext.get(f.id);
                                    if (!ctx) return null;
                                        return ctx.status === 'within'
                                            ? <p className="text-sm font-bold text-emerald-400/80 uppercase tracking-wide">{ctx.count} find{ctx.count !== 1 ? 's' : ''} recorded here — signal supported</p>
                                            : <p className="text-sm font-bold text-emerald-400/80 uppercase tracking-wide">{ctx.count} find{ctx.count !== 1 ? 's' : ''} recorded nearby</p>;
                                    })()}
                                {f.isHighConfidenceCrossing && (
                                    <div className="bg-blue-600/30 p-2 rounded-xl border border-blue-400/70 animate-pulse">
                                        <p className="text-sm font-bold uppercase text-white text-center tracking-[0.18em]">Likely historic crossing point</p>
                                </div>
                            )}
                            {f.explanationLines && f.explanationLines.length > 0 && (
                                <div className="border-t border-white/8 pt-3">
                                    <p className="text-sm font-bold text-white/75 uppercase tracking-wide mb-2">Why this matters</p>
                            ...
                            <div className="border-t border-emerald-500/15 pt-2">
                                <p className="text-sm font-bold text-emerald-500/70 uppercase tracking-[0.12em] mb-1">Target focus</p>
                            ...
                            {f.aimInfo && (
                                <div className="p-2 rounded-xl border bg-amber-500/10 border-amber-400/30">
                                    <p className="text-sm font-bold uppercase text-amber-300 leading-tight mb-0.5">Historic verification</p>
                                    <p className="text-sm font-bold text-white/80 leading-tight">{f.aimInfo.type} · {f.aimInfo.period}</p>
                                </div>
                            )}
                            {f.routeAssessment?.relationship === 'route_edge_activity_candidate' && (
                                <div className="p-2 rounded-xl border bg-sky-500/10 border-sky-400/30">
                                    <p className="text-sm font-bold uppercase text-sky-300 leading-tight mb-0.5">Route-Edge Signal</p>
                                    <p className="text-sm font-bold text-white/80 leading-tight">This signal sits beside, not on, a mapped route. It may reflect older movement or route-edge activity.</p>
                                </div>
                            )}
                            {f.routeAssessment?.relationship === 'historic_movement_candidate' && (
                                <div className="p-2 rounded-xl border bg-amber-500/10 border-amber-400/30">
                                    <p className="text-sm font-bold uppercase text-amber-300 leading-tight mb-0.5">Movement Corridor</p>
                                    <p className="text-sm font-bold text-white/80 leading-tight">Multiple signals suggest this may relate to an older movement corridor rather than a modern track.</p>
                                </div>
                            )}
                            {f.routeAssessment?.relationship === 'possible_modern_route_noise' && (
                                <div className="p-2 rounded-xl border bg-amber-500/15 border-amber-400/40">
                                    <p className="text-sm font-bold uppercase text-amber-300 leading-tight mb-0.5">Proximity Caution</p>
                                    <p className="text-sm font-bold text-white/80 leading-tight">This signal lies close to a mapped modern track or road edge. Treat with additional caution.</p>
                                </div>
                            )}
                                <div className="border-t border-white/8 pt-2">
                                    <button type="button"
                                        aria-expanded={expandedTargetId === f.id} onClick={() => toggleTargetDetails(f.id)}
                                        className="text-sm font-bold text-amber-400 hover:text-amber-300 transition-colors cursor-pointer flex items-center gap-1"
                                    >
                                    ...
                                    {expandedTargetId === f.id && (
                                        <div id={`mobile-target-details-${f.id}`} className="mt-3 space-y-3 animate-in fade-in duration-200">
                                            <div>
                                            <p className="text-sm font-bold text-white/75 uppercase tracking-[0.15em] mb-1">Summary</p>
                                            <p className="text-xs text-white/85 leading-relaxed">{tInterp.summary}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm font-bold text-white/75 uppercase tracking-[0.15em] mb-1">Why it stands out</p>
                                            <p className="text-xs text-white/85 leading-relaxed">{tInterp.whyItStandsOut}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm font-bold text-white/75 uppercase tracking-[0.15em] mb-1">How to approach it</p>
                    ...
                    {neutralMobileGeologyAvailable && (
                        <button
                            ...
                            className={`w-full rounded-xl border px-3 py-2.5 text-sm font-bold uppercase tracking-wide transition-colors ${expandedGeologyId === 'scan' ? 'bg-emerald-500/15 border-emerald-400/30 text-emerald-300' : 'bg-white/[0.04] border-sky-400/20 text-sky-300'}`}
                        >
                            Geology
                        </button>
                    )}
                ...
                {selectedMonument !== undefined && !selectedUserFind && !selectedPASFind && !selectedId && (
                    <div className="space-y-3">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <p className="text-sm font-bold text-stone-400/70 uppercase tracking-[0.2em] mb-1">Scheduled Monument</p>
                                {selectedMonument && <h3 className="text-base font-bold text-white/90 tracking-tight leading-tight">{selectedMonument}</h3>}
                            </div>
                            <button aria-label="Close selection" onClick={() => setSelectedMonument(undefined)} className="min-h-11 min-w-11 bg-white/[0.04] hover:bg-white/[0.08] text-white/75 hover:text-white rounded-full p-1.5 transition-colors border border-white/10 shrink-0">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                            </button>
                        </div>
                        <div className="rounded-xl bg-stone-900/40 border border-stone-700/40 p-3 space-y-2">
                ...
                {selectedMonument === undefined && selectedHotspotId && (() => {
                    ...
                    return (
                        <div className="space-y-3">
                            <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                    <p className="text-sm font-bold text-white/75 uppercase tracking-wide mb-0.5">{HOTSPOT_TITLES[h.classification]} · Hotspot {h.number}</p>
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <p className={`text-base font-bold leading-tight ${hStrengthColour}`}>{hierarchy.signalStrength}</p>
                                        {isPrimaryHotspot && <span className="bg-emerald-500/15 border border-emerald-400/30 text-emerald-200 px-1.5 py-0.5 rounded-full text-sm font-bold uppercase tracking-wide">Priority</span>}
                                    </div>
                                </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button
                                            ...
                                            className={`text-sm font-bold uppercase tracking-wide transition-colors ${expandedGeologyId === `hotspot:${h.id}` ? 'text-emerald-300' : 'text-sky-300 hover:text-sky-200'}`}
                                        >
                                            Geology
                                        </button>
                                        <button aria-label="Close selection" onClick={() => setSelectedHotspotId(null)} className="min-h-11 min-w-11 bg-white/[0.04] hover:bg-white/[0.08] text-white/75 hover:text-white rounded-full p-1.5 transition-colors border border-white/10">
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                        </button>
                                    </div>
                                </div>
                            <div className="space-y-2">
                                <div>
                                    <p className="text-sm font-bold text-white/75 uppercase tracking-[0.18em] mb-0.5">Why it matters</p>
                                    <p className="text-sm font-bold text-white/85 leading-snug">{hierarchy.whatWasObserved}</p>
                                </div>
                                <div>
                                    <p className="text-sm font-bold text-emerald-400/60 uppercase tracking-[0.18em] mb-0.5">Interpretive cue</p>
                            ...
                            {(h.secondaryTag || h.isOnCorridor || (h.linkedCount ?? 0) > 0) && (
                                <div className="flex items-center gap-2 flex-wrap">
                                    {h.secondaryTag && <span className="text-sm font-bold text-amber-300/60 uppercase tracking-wide">{h.secondaryTag}</span>}
                                    {h.isOnCorridor && <span className="text-sm font-bold text-emerald-500/60 uppercase tracking-wide">On corridor</span>}
                                    {(h.linkedCount ?? 0) > 0 && <span className="text-sm font-bold text-white/75 uppercase tracking-wide">Linked to {h.linkedCount} nearby</span>}
                                </div>
                            )}
                            ...
                            {(() => {
                                ...
                                return (
                                    ...
                                    {h.isHighConfidenceCrossing && <div className="bg-blue-600/30 p-2 rounded-xl border border-blue-400/70 animate-pulse"><p className="text-sm font-bold uppercase text-white text-center tracking-[0.18em]">Likely historic crossing point</p></div>}
                            {h.disturbanceRisk === 'High' && <div className="bg-red-500/15 p-2 rounded-xl border border-red-400/30"><p className="text-sm font-bold uppercase text-red-300 tracking-wide">Disturbed ground — interpret with caution</p></div>}
                            <div className="border-t border-white/8 pt-3">
                                <p className="text-sm font-bold text-white/75 uppercase tracking-wide mb-2">Evidence</p>
                                <div className="space-y-1.5">
                            ...
                            {evidenceProvenanceLabels(h.provenance).length > 0 && (
                                <details className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
                                    ...
                                    <ul className="space-y-1.5 pb-1">
                                        {evidenceProvenanceLabels(h.provenance).map(label => <li key={label} className="text-xs font-medium leading-snug text-white/75">{label}</li>)}
                                    </ul>
                                </details>
                            )}
                            {(() => {
                                ...
                                return (
                                    <div className={`mt-2 rounded-xl border px-3 py-2.5 space-y-1 ${isAssociated ? 'bg-emerald-500/8 border-emerald-500/20' : 'bg-sky-500/6 border-sky-500/15'}`}>
                                        <div className="flex items-center gap-2">
                                            <span className={`text-sm font-bold uppercase tracking-wide shrink-0 ${isAssociated ? 'text-emerald-400' : 'text-sky-400'}`}>
                                                Your finds
                                            </span>
                                            <span className={`text-sm font-bold ${isAssociated ? 'text-emerald-200' : 'text-sky-200'}`}>
                                                {feedback.label}
                                            </span>
                                        </div>
                                        <p className="text-sm font-bold text-white/75 leading-snug">{feedback.note}</p>
                                    </div>
                                );
                            })()}
                            {(() => {
                                ...
                                return (
                                    <div className="mt-2 rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-2.5 space-y-1">
                                        ...
                                        <p className="text-sm font-bold text-white/75 leading-snug">{persisted.note}</p>
                                    </div>
                                );
                            })()}
                            {h.suggestedFocus && (
                                <div className="pt-2 border-t border-emerald-500/15">
                                    <p className="text-sm font-bold text-emerald-500/70 uppercase tracking-[0.12em] mb-1">Field focus</p>
                                    <p className="text-sm font-bold text-emerald-300 leading-snug">{h.suggestedFocus}</p>
                                </div>
                            )}
                                <div className="pt-2 border-t border-white/8">
                                    <button type="button" aria-expanded={expandedInterpretationId === h.id} onClick={() => toggleHotspotDetails(h.id)} className="text-sm font-bold text-amber-400 hover:text-amber-300 cursor-pointer flex items-center gap-1">
                                    ...
                                    const breakdown = [{ label: 'Anomaly', val: h.metrics.anomaly, cap: 30 }, { label: 'Context', val: h.metrics.context, cap: 25 }, { label: 'Convergence', val: h.metrics.convergence, cap: 20 }, { label: 'Behaviour', val: h.metrics.behaviour, cap: 15 }];
                                        return (
                                            <div id={`mobile-hotspot-details-${h.id}`} className="mt-3 space-y-3 animate-in fade-in duration-200">
                                                <p className="text-sm font-bold text-white/75 uppercase tracking-[0.2em]">{getInterpretationLabel(h.confidence)}</p>
                                            ...
                                            <div className="space-y-1.5 pt-2 border-t border-white/10">
                                                {breakdown.map(({ label, val, cap }) => (
                                                    <div key={label} className="flex items-center gap-2">
                                                        <span className="text-sm text-white/75 w-16 shrink-0">{label}</span>
                                                        ...
                                                        <span className="text-sm text-white/75 w-14 text-right shrink-0">{getSignalBand(val, cap)}</span>
                                                    </div>
                                                ))}
                                                {h.metrics.penalty !== 0 && <p className="text-sm text-white/75 mt-1">Modern disturbance or noise was discounted before interpretation.</p>}
                                            </div>
                                        </div>
                                    );
                                    })()}
                                </div>
                                    <p className="text-center text-sm text-white/75 italic">Highlights historic activity — not guaranteed finds.</p>
                        </div>
                    );
                })()}
                {!historicMode && <FieldGuideNotice />}
            </div>
        </div>
        {activeMobileGeologyTitle && (
            <div className="absolute bottom-3 left-3 right-3 z-[95] max-h-[65dvh] overflow-y-auto overscroll-contain animate-in fade-in zoom-in-95 duration-150">
                <div className="relative rounded-2xl border border-emerald-400/25 bg-black/95 shadow-2xl backdrop-blur-xl">
                    ...
                    <button
                        ...
                        className="absolute right-3 top-3 z-10 min-h-11 min-w-11 grid place-items-center bg-white/[0.04] hover:bg-white/[0.08] text-white/75 hover:text-white rounded-full p-1.5 transition-colors border border-white/10"
                    >
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </button>
                    <GeologyContextCard
