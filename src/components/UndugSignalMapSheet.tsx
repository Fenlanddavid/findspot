...
// Tapping the (static) marker opens the signal detail sheet on top.
...
// Inline SVG string for the marker DOM element (kebab-case SVG attributes).
const MARKER_SVG = `<svg width="22" height="22" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function UndugSignalMapSheet({ signal, onClose, onConvertToFind }: Props) {
  ...
  React.useEffect(() => {
    ...
    map.once('load', () => {
      // Marker element — stopPropagation prevents map re-centering on tap.
      const el = document.createElement('div');
      el.setAttribute('aria-label', 'Un-dug signal — tap to view details');
      el.style.cssText = [
