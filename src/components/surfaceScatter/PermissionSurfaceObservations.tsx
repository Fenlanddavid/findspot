      ...
      {summary.observationCount > 0 && (
        <div className="mt-3 rounded-xl border border-sky-200 bg-white/80 p-3 dark:border-sky-800/70 dark:bg-gray-950/25">
          ...
          {showMap && <div className="mt-3">
            ...
            {mapSelectedObservation && <button type="button" onClick={() => setDetailId(mapSelectedObservation.id)} className="mt-2 flex min-h-12 w-full items-center justify-between rounded-xl border border-sky-200 bg-sky-50 px-3 text-left dark:border-sky-800 dark:bg-sky-950/20"><span><span className="block text-xs font-black">{mapSelectedObservation.observationKind === 'iron_patch' ? 'Iron / junk patch' : `${SURFACE_MATERIAL_LABELS[mapSelectedObservation.material]} · ${mapSelectedObservation.periodImpression === 'unknown' ? 'Period not sure' : SURFACE_PERIOD_LABELS[mapSelectedObservation.periodImpression]}`}</span><span className="block text-3xs font-bold text-gray-500">{mapSelectedObservation.observationKind === 'iron_patch' ? (mapSelectedObservation.extent?.replaceAll('_', ' ') ?? 'Approximate spread') : mapSelectedObservation.abundance} · {mapSelectedObservation.gpsAccuracyM == null ? 'GPS accuracy unknown' : `GPS ±${Math.round(mapSelectedObservation.gpsAccuracyM)} m`} · Observed by you</span></span><span className="text-xs font-black text-sky-700 dark:text-sky-300">View details</span></button>}
          </div>}
          {clusters.length > 0 && (
