...
import { SURFACE_MATERIAL_LABELS } from '../../services/surfaceScatter';
import { BASEMAP_LAYERS, BASEMAP_SOURCES } from '../permission/basemaps';
...
export function SurfaceObservationsMap({
  ...
  useEffect(() => {
    ...
    markersRef.current = observations.map(observation => {
      ...
      button.type = 'button';
      button.title = `${SURFACE_MATERIAL_LABELS[observation.material]} — ${observation.abundance}`;
      button.setAttribute('aria-label', button.title);
      button.textContent = style.glyph;
      button.style.cssText = `width:${size}px;height:${size}px;border-radius:999px;border:2px solid white;background:${style.color};color:white;font:800 ${size < 22 ? 8 : 9}px system-ui;box-shadow:0 2px 8px rgba(0,0,0,.4);cursor:pointer;`;
      if (observation.id === selectedId) button.style.outline = '3px solid rgba(56,189,248,.75)';
