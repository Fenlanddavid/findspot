...
export default function FieldReportModal({ sessionId, onClose }: Props) {
  ...
  async function buildPDFBlob(): Promise<{ blob: Blob; filename: string }> {
    ...
    const isGroupReport = data!.permission.type === "rally" || !!data!.permission.isClubDayMember;
    const reference = makeReportReference(isGroupReport ? "RALLYVISIT" : "VISIT", data!.session.id, generatedAt);
    applyReportPdfMetadata(pdf, {
      title: `${isGroupReport ? "Club/Rally Session Report" : "Session Report"} - ${data!.permission.name}`,
      subject: `${reference} generated by FindSpot from local session data.`,
      reference,
      generatedAt,
    });
